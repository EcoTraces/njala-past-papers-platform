...
| `..._analytics_trends.sql` | `analytics_daily_trends(p_days)` - SECURITY INVOKER, day-bucketed `GROUP BY` (via `generate_series()` + per-table `date` casts, left-joined) across `examination_papers`/`paper_views`/`paper_downloads`/`practice_sessions` for the last 1-365 days (clamped both in-function and at the API layer). Same class of problem as `search_examination_papers()`/`admin_dashboard_stats()` - PostgREST can't express a multi-table day-bucketed aggregate. No new RLS policies needed: `paper_views_select_own`/`paper_downloads_select_own` already grant `auth_is_staff()` full visibility, the only roles `GET /api/analytics/trends` is reachable by. Adds supporting indexes (`idx_paper_views_viewed_at`, `idx_paper_downloads_downloaded_at`, a partial `idx_practice_sessions_submitted_at` on `status = 'SUBMITTED'`) for the new range-filter query pattern. |
